...
class Marine(AttackUnit):
    ...
    # 스팀팩 키등 : 일정시간동안 이동속도 및 공격속도를 증가, 체력 10 감소
    def stimpack(self):
        if self.hp > 10:
            self.hp -= 10
...
class Tank(AttackUnit):
    ...
    def set_seize_mode(self):
        ...
        if self.seize_mode == False:
            print
            {"{0} : 시즈모드로 전환합니다.".format(self.name)}
            self.damage *= 2
...
class Flyable:
    ...
    def fly(self, name, location):
        print(f"{self.name} : {location} 방향으로 날아갑니다. [속도 {self.flyng_speed}]")
